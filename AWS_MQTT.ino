...
//Callback: funciona cuando ocurra una interrupcion, en este caso cuando llegue un mensaje
...
void setup() {
  ...
  //*****************************
  // Cert leer archivo
  File file4 = SPIFFS.open("/57e68-certificate.pem.crt", FILE_READ);
  if (!file4) {
    Serial.println("No se pudo abrir el archivo para leerlo");
    return;
  }
  ...
  File file6 = SPIFFS.open("/57e68-private.pem.key", FILE_READ);
  if (!file6) {
    Serial.println("No se pudo abrir el archivo para leerlo");
    return;
  }
...
void loop() {
  ...
  if (now - lastMsg > 5000) {
    ...
    //=============================================================================================    
    /*
     * Forma rudimentaria de generar el json:
